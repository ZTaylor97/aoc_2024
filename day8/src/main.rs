    ...
    for (key, vec) in map {
        for (i, (row, col)) in vec.iter().enumerate() {
            for j in i..vec.len() {
                ...
                if test_node(node_1, bounds) {
                    antinodes.insert(node_1);
                }

                if test_node(node_2, bounds) {
...
fn test_node(node: (i32, i32), bounds: (u32, u32)) -> bool {
